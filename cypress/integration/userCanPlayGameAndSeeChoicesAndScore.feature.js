describe("Visitor can visit page and see content", () => {
  ...
  it("visitor can see content on page ", () => {
    ...
    cy.get("h1").should(
      "contain",
      "RPS - A GAME THAT WILL DECIDE THE FATE OF MANKIND"
    );
    cy.get("h1").should(
      "contain",
      "WELCOME HUMAN. YOUR CHOICE WILL DETERMINE THE FATE OF PLANET EARTH. WHAT CHOOSE YOU?"
    );
  });
});
...
describe("User can see the game score", () => {
  it("displays a score", () => {
    cy.get("h4#playerscore");
    cy.get("h4#computerscore");
  });
});
